...
function DictsPage() {
  ...
  const handleOpenCreate = () => {
    ...
    form.resetFields();
    form.setFieldsValue({ dictSort: 0 });
    setOpen(true);
  };
  ...
  return (
    <>
      <ModulePage<DictRecord>
        ...
        loading={loading}
        tableTitle="字典项列表"
        searchPlaceholder="搜索名称、类型、标签或键值"
        getSearchText={(record) => `${record.dictName ?? ''} ${record.dictType} ${record.dictLabel} ${record.dictValue} ${record.remark ?? ''}`}
