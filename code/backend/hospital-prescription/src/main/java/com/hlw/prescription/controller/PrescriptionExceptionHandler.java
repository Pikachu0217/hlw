...
/**
 * 处方服务异常处理器，统一返回业务异常和未知异常。
 */
@RestControllerAdvice
public class PrescriptionExceptionHandler {
    private static final Logger log = LoggerFactory.getLogger(PrescriptionExceptionHandler.class);
    ...
    public R<Void> handleBizException(BizException exception) {
        log.warn("处方服务业务异常，code={}，message={}", exception.getCode(), exception.getMessage());
    ...
    public R<Void> handleException(Exception exception) {
        log.error("处方服务未知异常", exception);
        return R.fail(500, "处方服务暂不可用");
    }
}
